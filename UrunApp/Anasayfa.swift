...
extension Anasayfa : UITableViewDelegate, UITableViewDataSource ,HucreProtocol {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        hucre.arkaPlanHucre.layer.cornerRadius = 10.0
    ...
    //tablo görünümü elde etmek için tableview açarız ve urunler dizimizi indexpath olarak sıralı tnaımlarız.
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath:  IndexPath) -> UISwipeActionsConfiguration? {
        let urun = urunlerListesi[indexPath.row]
        //hucrenın içinde sil butonu böyle oluşturulur.
        let silAction = UIContextualAction(style: .destructive, title: "Sil") {
            contextualAction , view, bool in
            print("\(urun.ad!) silindi ")
        }
        //hucrenin içinde duzenle butonu böyle oluşturulur.
    ...
    //hucrenin uzerine tıklanma işlemini tanımlıyoruz.Bu bize seçilen satırı indexpath olarak veriyor.
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let urun = urunlerListesi[indexPath.row]
        performSegue(withIdentifier: "toDetay", sender: urun) //Detay sayfasını oluşturduktan sonra hücreye tıklandıktan sonra gitmesi gereken yeri ifade ediyorum.
    }
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        //geçiş yapacağım sayfayı filtreleyip geçiş bilgilerini yolluyorum.
